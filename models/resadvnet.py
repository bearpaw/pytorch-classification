...
class StackedAdversary(nn.Module):
    def __init__(self, block, p, t, r, planes, num_stacks=3, depth=3):
        super(StackedAdversary, self).__init__()
        self.num_stacks = num_stacks
        # self.attention = Attention(block, p, t, r, planes, depth)
        # self.attention = SoftmaxAttention(planes)
        attentions = []
        for s in range(0, self.num_stacks):
            attentions.append(SoftmaxAttention(planes, residual=False))
...
class ResAdvNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.adv1 = StackedAdversary(block, 1, 2, 1, 16 * block.expansion, 
                            num_stacks=5, depth=3)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2)
        self.adv2 = StackedAdversary(block, 1, 2, 1, 32 * block.expansion, 
                            num_stacks=5, depth=3)
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2)
        self.adv3 = StackedAdversary(block, 1, 2, 1, 64 * block.expansion, 
                            num_stacks=5, depth=3)
